...
int main() {
	...
	char outputFileName[30] = "BlueEdge_SetD.dat"; //File with organized output
		...
		if(inFile.good()){
			...
			if(LINAi != 0){
				if(growth[LINAi] < 0. && growth[LINAi-1] > 0.){
					outFile<<setw(20)<<Mod[i]<<setw(30)<<z[i]<<setw(30)<<x[i]<<setw(30)<<M[i]<<setw(30)<<L[i]<<setw(30)<<T[i]<<endl;
